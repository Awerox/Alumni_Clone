...
export default async function MentoringPage({
  ...
  searchParams: { q?: string; diplome?: string; promotion?: string; campus?: string; type?: string }
}) {
  const payload = await getPayload({ config: configPromise })

  // 🔐 1. Récupération de la session utilisateur via les headers (await obligatoire en App Router)
  ...
  // 🎯 2. Récupération de la fiche Alumni de l'utilisateur connecté
  ...
  // 🔍 3. Préparation des filtres pour l'annuaire des mentors
  const queryConstraints: any = {
    isMentor: { equals: true },
    id: { not_equals: user.id },
  }

  if (searchParams.diplome) {
    queryConstraints.diplome = { like: searchParams.diplome }
  }
  if (searchParams.campus) {
    queryConstraints.campus = { equals: searchParams.campus }
  }
  if (searchParams.promotion) {
    queryConstraints.promotion = { equals: Number(searchParams.promotion) }
  }
  if (searchParams.type) {
    queryConstraints.typeMentorat = { equals: searchParams.type }
  }
  if (searchParams.q) {
    queryConstraints.or = [
      { prenom: { like: searchParams.q } },
      { nom: { like: searchParams.q } },
      { poste: { like: searchParams.q } },
    ]
  }
          ...
          <form method="GET" action="/mentoring" className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 text-xs font-bold text-gray-500">
              <input
                ...
                placeholder="🔍 Mots-clés (Nom, poste...)"
              ...
              <select
                name="diplome"
                defaultValue={searchParams.diplome || ''}
                className="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg outline-none font-medium text-gray-700"
              >
              ...
              <select
                name="promotion"
                defaultValue={searchParams.promotion || ''}
                className="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg outline-none font-medium text-gray-700"
              >
            ...
            <div className="flex justify-between items-center pt-3 border-t border-gray-100 text-[11px] font-black uppercase">
              <Link href="/mentoring" className="text-gray-400 hover:text-gray-600">
                🔀 Effacer les filtres
              </Link>
              <button
                type="submit"
                className="bg-[#7C3AED] text-white px-5 py-2 rounded-xl hover:bg-purple-700 transition-colors"
              >
                Filtrer les mentors
              </button>
            </div>
          ...
          {mentorsList.docs.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {mentorsList.docs.map((mentor: any) => (
                <div
                  key={mentor.id}
                  className="border border-gray-100 rounded-xl p-4 flex flex-col justify-between space-y-4 hover:border-purple-200 transition-all"
                >
            ...
            <div className="text-center py-12 space-y-2">
              ...
              <p className="text-[11px] text-gray-400 max-w-xs mx-auto font-medium">
                Modifiez vos sélections ou écrivez un mot-clé différent pour rechercher d'autres
                profils.
              </p>
            </div>
          )}
