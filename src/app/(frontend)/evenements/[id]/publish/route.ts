...
async function getUser(req: NextRequest, secret: string) {
  ...
  try { const d = verify(token, secret) as any; return d?.collection === 'alumni' || d?.collection === 'users' ? d : null } catch { return null }
}
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Vérifier que c'est l'organisateur ou un admin
