...
export default async function JobsPage({ searchParams }: PageProps) {
  ...
  // 🛡️ Blocage de sécurité si l'utilisateur n'est pas connecté
  if (!user) {
    return (
  ...
  if (currentTab === 'actives') {
    andConditions.push({ statut: { equals: 'publie' } })
  } else if (currentTab === 'attente') {
    andConditions.push({ statut: { equals: 'attente' } })
  } else if (currentTab === 'brouillon') {
    andConditions.push({ statut: { equals: 'brouillon' } })
  }

  // Filtre par mots-clés (poste ou entreprise)
  if (resolvedSearchParams.q) {
    andConditions.push({
      or: [
        { poste: { like: resolvedSearchParams.q } },
        { entreprise: { like: resolvedSearchParams.q } },
      ],
    })
  }

  // Filtre par localisation
  if (resolvedSearchParams.loc) {
    andConditions.push({ localisation: { like: resolvedSearchParams.loc } })
  }

  // Filtre par secteur
  if (resolvedSearchParams.secteur) {
    andConditions.push({ secteur: { equals: resolvedSearchParams.secteur } })
  }

  // Filtre par type de contrat
  if (resolvedSearchParams.contrat) {
    andConditions.push({ typeContrat: { equals: resolvedSearchParams.contrat } })
  }

  // Filtre par rémunération
  if (resolvedSearchParams.remuneration) {
    andConditions.push({ remuneration: { equals: resolvedSearchParams.remuneration } })
  }

  // Filtre par expérience
  if (resolvedSearchParams.experience) {
    andConditions.push({ experience: { equals: resolvedSearchParams.experience } })
  }

  // Filtre par recruteur (mes offres uniquement)
  ...
  try {
    ...
    console.error('Erreur de requête SQL sur les offres:', error)
    schemaConflictDetected = true
  }

  // Mappers d'affichage de l'interface en français
  const remLabels: any = {
  ...
  return (
    <div className="bg-gray-50/50 min-h-screen py-10 px-4 sm:px-6 lg:px-8 text-left font-sans">
      <div className="max-w-6xl mx-auto space-y-6">
        {/* Bandeau d'avertissement de conflit de schéma d'Enum local */}
        {schemaConflictDetected && (
          <div className="bg-red-50 border border-red-200 text-red-700 p-4 rounded-xl text-xs font-bold shadow-xs">
            ⚠️ <span className="uppercase">Erreur de base de données :</span> La valeur d'état{' '}
            <code className="bg-red-100 px-1 py-0.5 rounded text-red-800">"attente"</code> n'est pas
            encore enregistrée dans l'Enum SQL local de votre machine. Exécutez{' '}
            <code className="bg-red-100 px-1 py-0.5 rounded text-red-800">
        ...
        <div className="bg-[#FEF3C7] border border-[#FDE68A] text-[#92400E] p-4 rounded-xl text-xs font-bold flex justify-between items-center shadow-2xs">
          <p>
            Vous n'avez pas encore activé la notification hebdomadaire des offres d'emploi. Vous
            pouvez l'activer par{' '}
            <Link href="/profile" className="underline font-black">
              ici
            </Link>
          </p>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-xs">
          <form
            method="GET"
            action="/jobs"
            className="flex flex-wrap items-center gap-3 text-xs font-bold text-gray-400"
          >
            ...
            <select
              name="secteur"
              defaultValue={resolvedSearchParams.secteur || ''}
            ...
            <select
              name="contrat"
              defaultValue={resolvedSearchParams.contrat || ''}
              className="px-3 py-2 bg-white border border-gray-200 rounded-lg outline-none text-gray-700 font-medium focus:border-emerald-500 cursor-pointer"
            >
            ...
            <select
              name="remuneration"
              defaultValue={resolvedSearchParams.remuneration || ''}
              className="px-3 py-2 bg-white border border-gray-200 rounded-lg outline-none text-gray-700 font-medium focus:border-emerald-500 cursor-pointer"
            >
            ...
            <input
              type="text"
              name="loc"
              defaultValue={resolvedSearchParams.loc || ''}
            ...
            <button
              type="submit"
              className="bg-emerald-500 text-white px-5 py-2 rounded-lg font-black uppercase shadow-2xs cursor-pointer hover:bg-emerald-600 transition-colors"
            >
              Filtrer
            </button>
            <Link
              href="/jobs"
              className="text-gray-400 hover:text-gray-600 text-[11px] uppercase ml-1 flex items-center"
            >
              Effacer
            </Link>
          </form>
        ...
        <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4">
          ...
          <div className="bg-amber-50/40 border border-amber-200/60 rounded-2xl p-1.5 flex items-center gap-1 shadow-sm text-[10px] font-black uppercase overflow-x-auto max-w-full">
            <Link
              href="/jobs?tab=actives"
              className={`px-4 py-2 rounded-xl transition-all ${currentTab === 'actives' ? 'bg-amber-400 text-gray-900 shadow-2xs' : 'text-gray-500 hover:text-gray-700'}`}
            >
              Offres actives
            </Link>
            <Link
              href="/jobs?tab=attente"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {offresDocs.map((offre: any) => {
              ...
              return (
                <div
                  key={offre.id}
                  className="bg-white border border-gray-100 rounded-3xl p-6 shadow-xs relative flex flex-col justify-between hover:shadow-md transition-all group hover:border-gray-300"
                >
                  <div className="space-y-5">
                    {/* Header de la carte : Logo et Titre */}
                    <div className="flex justify-between items-start gap-4">
                      <div className="flex items-center gap-4">
                        {logoUrl ? (
                          <img
                            src={logoUrl}
                            alt=""
                            className="w-16 h-16 rounded-2xl border border-gray-100 object-contain bg-white shadow-xs p-1"
                          />
                        ) : (
                          <div className="w-16 h-16 bg-gray-50 border border-gray-100 text-gray-400 rounded-2xl flex items-center justify-center text-2xl shadow-xs">
                            🏢
                          </div>
                        )}
                        <div>
                      ...
                      <span className="text-[10px] font-black text-gray-400 bg-gray-100 px-2 py-1 rounded-md whitespace-nowrap">
                        {cleanDate}
                      </span>
                    </div>

                    {/* Contenu central : Grille d'informations esthétique */}
                    <div className="grid grid-cols-2 gap-2 text-[11px] font-bold">
                      <div className="bg-emerald-50/60 border border-emerald-100/50 rounded-xl p-2.5 flex flex-col justify-center">
                        ...
                        <span className="text-emerald-700 uppercase font-black tracking-wide mt-0.5">
                      ...
                      <div className="bg-blue-50/60 border border-blue-100/50 rounded-xl p-2.5 flex flex-col justify-center">
                        ...
                        <span className="text-blue-700 font-black mt-0.5 flex items-center gap-1">
                          📅 {dateDebutText}
                        </span>
                      </div>
                      ...
                      <div className="bg-gray-50 border border-gray-100 rounded-xl p-2.5 col-span-2 flex justify-between items-center">
                        <div>
                          ...
                          <span className="text-gray-800 font-semibold truncate max-w-[240px] inline-block mt-0.5">
                            {sectLabels[offre.secteur] || offre.secteur}
                          </span>
                        </div>
                        <div className="text-right">
                          ...
                          <span className="text-gray-800 font-semibold inline-block mt-0.5">
                      ...
                      {offre.dateLimite && (
                        <div className="bg-amber-50/50 border border-amber-100/40 rounded-xl p-2 col-span-2 text-center text-amber-800 text-[10px]">
                          ⏳ Postuler avant le :{' '}
                          <span className="font-black">{dateLimiteText}</span>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="pt-4 mt-4 border-t border-gray-100 flex justify-between items-center text-[10px] uppercase font-black tracking-widest">
                    ...
                    <Link
                      href={`/jobs/${offre.id}`}
                      className="bg-gray-900 hover:bg-emerald-600 text-white hover:text-white px-3 py-1.5 rounded-lg transition-all flex items-center gap-1 normal-case font-bold shadow-2xs"
                    >
                      Voir l'offre ➔
                    </Link>
                  </div>
