...
export default function MessagesAndChatPage() {
  ...
  // Charger l'historique depuis la BDD quand on sélectionne un utilisateur
  const loadHistory = async (otherUser: OnlineUser) => {
    if (!me) return
  ...
  // Quand on clique sur un utilisateur, charger l'historique
  const handleSelectUserForMP = (user: OnlineUser) => {
    setSelectedUserForMP(user)
    setDirectMessages([]) // Reset avant chargement
    loadHistory(user)
  }
  ...
  useEffect(() => {
    ...
    eventSource.onmessage = (event) => {
      ...
      if (data.type === 'presence-full') {
        const filtrés: OnlineUser[] = (data.users as OnlineUser[]).filter(
          (u) => String(u.id) !== String(me.id)
        )
        ...
        // Comparer en string pour éviter number vs string
        if (String(data.to) === String(me.id) || String(data.from) === String(me.id)) {
          setDirectMessages((prev) => {
            // Éviter les doublons si le message vient de moi (déjà ajouté optimistement)
            const exists = prev.some(
              (m) => m.from === data.from && m.to === data.to && m.text === data.text && m.time === data.time
            )
  ...
  const handleSendLiveChatMessage = async (e: React.FormEvent) => {
    ...
    try {
      await fetch('/api/chat/stream', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ type: 'msg-public', user: `${me.prenom} ${me.nom}`, text: typedChatMessage, time: new Date().toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' }) }),
      })
      setTypedChatMessage('')
    } catch (err) { console.error(err) }
  }
  ...
  const handleSendPrivateMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        // Ajout optimiste immédiat pour l'expéditeur
    ...
    <div className="bg-[#F8FAFC] min-h-screen font-sans text-left">
      <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
        ...
        <div className="flex bg-white p-1.5 border border-gray-200 rounded-2xl w-full max-w-md shadow-2xs font-black uppercase text-[10px] tracking-wider">
          <button onClick={() => setActiveTab('forum')} className={`flex-1 py-2.5 rounded-xl transition-all cursor-pointer ${activeTab === 'forum' ? 'bg-purple-600 text-white' : 'text-gray-500 hover:text-gray-800'}`}>🏛️ Forum</button>
          <button onClick={() => setActiveTab('chat')} className={`flex-1 py-2.5 rounded-xl transition-all cursor-pointer ${activeTab === 'chat' ? 'bg-gray-900 text-white' : 'text-gray-500 hover:text-gray-800'}`}>🟢 Chat en direct</button>
          <button onClick={() => setActiveTab('mp')} className={`flex-1 py-2.5 rounded-xl transition-all cursor-pointer ${activeTab === 'mp' ? 'bg-emerald-600 text-white' : 'text-gray-500 hover:text-gray-800'}`}>🔒 Messages Privés</button>
        </div>
        ...
        {activeTab === 'mp' && (
          <div className="bg-white border border-gray-200 rounded-3xl p-6 shadow-xs grid grid-cols-1 md:grid-cols-12 gap-6 h-[550px] overflow-hidden">
            <div className="md:col-span-4 border-r border-gray-100 pr-2 overflow-y-auto space-y-2 h-full">
              ...
              {onlineUsers.length > 0 ? onlineUsers.map((user, i) => (
                <div key={i} onClick={() => handleSelectUserForMP(user)} className={`p-3 rounded-xl border text-left cursor-pointer transition-all flex items-center gap-3 ${selectedUserForMP?.id === user.id ? 'border-emerald-200 bg-emerald-50/30' : 'border-gray-50 hover:bg-gray-50'}`}>
            ...
            <div className="md:col-span-8 flex flex-col justify-between h-full overflow-hidden">
              {selectedUserForMP ? (
                <>
                  ...
                  <div className="flex-1 overflow-y-auto py-4 space-y-3 pr-1">
                    {loadingHistory ? (
                      ...
                      <div className="flex items-center justify-center h-full">
                        <p className="text-xs text-gray-400 italic">Aucun message. Commencez la conversation !</p>
                      </div>
                    ) : (
                  ...
                  <form onSubmit={handleSendPrivateMessage} className="pt-3 border-t border-gray-100 flex items-center gap-2">
                    ...
                    <input type="text" placeholder={`Envoyer un message à ${selectedUserForMP.name}...`} value={typedPrivateMessage} onChange={(e) => setTypedPrivateMessage(e.target.value)} disabled={isUploadingFile} className="flex-1 px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-xs font-medium outline-none focus:bg-white focus:border-emerald-500 shadow-3xs" />
                ...
                <div className="h-full flex items-center justify-center text-gray-400 text-xs italic">Sélectionnez un membre connecté à gauche pour démarrer une conversation.</div>
              )}
            </div>
          </div>
        )}
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2 pt-1">
                  {Object.entries(categoryBadges).map(([key, value]) => (
                    <label key={key} className={`p-2.5 border rounded-xl flex items-center gap-2 cursor-pointer transition-all normal-case font-bold text-gray-700 ${newTopicCategorie === key ? 'border-purple-500 bg-purple-50/50 shadow-3xs' : 'border-gray-200 bg-white hover:bg-gray-50'}`}>
